...
public class FragNavController {
  ...
  private static final String EXTRA_TAG_COUNT =
      FragNavController.class.getName() + ":EXTRA_TAG_COUNT";
  private static final String EXTRA_SELECTED_TAB_INDEX =
      FragNavController.class.getName() + ":EXTRA_SELECTED_TAB_INDEX";
  private static final String EXTRA_CURRENT_FRAGMENT =
      FragNavController.class.getName() + ":EXTRA_CURRENT_FRAGMENT";
  private static final String EXTRA_FRAGMENT_STACK =
      FragNavController.class.getName() + ":EXTRA_FRAGMENT_STACK";

  private final List<Stack<Fragment>> mFragmentStacks;
  private final FragmentManager mFragmentManager;
  @TabIndex int mSelectedTabIndex = -1;
  ...
  public FragNavController(Bundle savedInstanceState, @NonNull FragmentManager fragmentManager,
      @IdRes int containerId, @NonNull List<Fragment> baseFragments) {
  ...
  public FragNavController(Bundle savedInstanceState, @NonNull FragmentManager fragmentManager,
      @IdRes int containerId, @NonNull List<Fragment> baseFragments, @Transit int transitionMode) {
    this(savedInstanceState, fragmentManager, containerId, baseFragments);
    mTransitionMode = transitionMode;
  }

  @IntDef({
      FragmentTransaction.TRANSIT_NONE, FragmentTransaction.TRANSIT_FRAGMENT_OPEN,
      FragmentTransaction.TRANSIT_FRAGMENT_CLOSE, FragmentTransaction.TRANSIT_FRAGMENT_FADE
  }) @Retention(RetentionPolicy.SOURCE) private @interface Transit {
  }
   ...
   */
  @TabIndex public void switchTab(@TabIndex int index) {
...
/*    public void showBottomSheet(BottomSheetDialogFragment bottomSheetDialogFragment) {
  ...
  @Nullable private Fragment getCurrentFrag() {
    ...
    //if not, try to pull it from the stack
      ...
      if (!fragmentStack.isEmpty()) {
        return mFragmentManager.findFragmentByTag(
            mFragmentStacks.get(mSelectedTabIndex).peek().getTag());
      } else {
        return null;
      }
    }
  ...
  private void onRestoreFromBundle(Bundle savedInstanceState, List<Fragment> baseFragments) {
    ...
    try {
      final JSONArray stackArrays =
          new JSONArray(savedInstanceState.getString(EXTRA_FRAGMENT_STACK));
